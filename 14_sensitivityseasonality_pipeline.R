...
dir.res <- file.path(dir.res, '02_pipeline')
...
dir.modelfit <- file.path(dir.res, '02a_modelFit')

local({
  source(file.path(dir.code, '02a_sensitivityseasonality_modelFit.R'))
})

# 2. sample crossbasis ----

dir.samplecb <- file.path(dir.res, '02b_sampleCrossbasisPosterior')

local({
  source(file.path(dir.code, '02b_sensitivityseasonality_sampleCrossbasisPosterior.R'))
})

# 3. define results ----

dir.finalres <- file.path(dir.res, '02c_resultsGenerating')

local({
  source(file.path(dir.code, '02c_sensitivityseasonality_resultsGenerating.R'))
})