...
calculate.areaResults <-
  function(data,
    ...
    for (j in seq_len(n.theta)) {
      ...
      # c. af
      if (return.AF) { af.array[, j] <- -expm1(-eta.j) } # (RR - 1) / RR = -[exp(-log{RR}) - 1]
        ...
        af.posterior.summary <-
          helper.processTimer(
            ...
            expr = {
              ...
              af.munc <- 
                data.table::copy(af.posterior) %>% 
                # 1. multiple theta columns by deaths = AF * Y per sample
                .[, (colnames.theta) := lapply(.SD, `*`, deaths), .SDcols = colnames.theta] %>% 
...
colours.municipality <- 
  colorRampPalette(
    RColorBrewer::brewer.pal(9, 'Spectral')
  )(nrow(municipality.details))
...
fp.munc.obsv <- file.path(dir.finalres, '01_municipality', '01_temperature_observered')
fs::dir_create(fp.munc.obsv)

if(
  file.path(fp.munc.obsv, c('summary_mmt.rds', 'summary_rr.rds', 'summary_af.rds')) %>% 
  file.exists() %>% 
  {any(!.)}
){
...
# # 3. RR plots ----
# 
# ## 3.1. temperature observed ----
# 
# ### 3.1.1. import data ----
# 
# rr <-
#   helper.objectLoadRun(
#     filepath = file.path(fp.munc.obsv, 'summary_rr.rds'),
#     FUN = NULL,
#     message.run = 'Need to run posterior for municipality at temperature observed',
#     message.load = 'Loading posterior for municipality RR at temperature observed'
#   )
# 
...
#   # lines
#   ggplot2::geom_line(mapping = ggplot2::aes(y = median, group = municipality_id, colour = municipality_id),
#                      linewidth = 0.5, alpha = 0.5, colour = 'lightskyblue2') +
#   # ggplot2::geom_line(data = rr.rgn,
#   #                    mapping = ggplot2::aes(y = median, group = Region, colour = Region),
#   #                    linewidth = 1, colour = 'darkorange3') +
#   # axis labels
...
#   ggplot2::theme(strip.background = element_rect(fill = 'grey80', color = NA),
#                  strip.text = element_text(color = 'black', face = 'bold'))
# 
# ggplot2::ggsave(filename = file.path(dir.finalres, 'rr_observed_region.png'),
#                 plot = plot.rr.rgn,
#                 height = plot.height,
#                 width = plot.width)
# 
...
#   #                    linewidth = 1.25, colour = 'darkorange3') +
#   # axis labels
#   ggplot2::scale_x_continuous(name = 'Temperature lag (0-3)') +
#   ggplot2::scale_y_continuous(name = 'Relative risk') +
#   # theme
#   ggplot2::theme_minimal(base_size = 11)
# 
# ggplot2::ggsave(filename = file.path(dir.finalres, 'rr_observed_country.png'),
#                 plot = plot.rr.ctry,
#                 height = plot.height,
#                 width = plot.width)
# 
...
# 
# ggplot2::ggsave(filename = file.path(dir.finalres, 'rr_percentile_region_map.png'),
#                 plot = plot.rr.rgn.map,
#                 height = plot.height,
#                 width = plot.width)
# 
# plot.rr.rgn.facet <-
...
#                                guide = ggplot2::guide_legend(override.aes = list(linewidth = 1.25))) +
#   # facet by Region
#   ggplot2::facet_wrap( ~ Region, ncol = 5, nrow = 4) +
#   # theme
#   ggplot2::theme_minimal(base_size = 11) +
#   ggplot2::theme(legend.position = 'none',
#                  # legend.position = c(0.02, 0.98),
...
#                  strip.text = element_text(color = 'black', face = 'bold'))
# 
# ggplot2::ggsave(filename = file.path(dir.finalres, 'rr_percentile_region_facet.png'),
#                 plot = plot.rr.rgn.facet,
#                 height = plot.height,
#                 width = plot.width)
# 
# nm.all.rgn <- rr.munc$Region %>% unique() %>% sort()
# plot.ymax <- ceiling(max(rr.munc$median)*10)/10
# plot.ymin <- floor(min(rr.munc$median)*10)/10
# plot.ybreak <- seq(from = plot.ymin, to = plot.ymax, by = 0.2)
# 
# lapply(
#   X = nm.all.rgn,
#   FUN = function(x) {
...
#     
#     plot.rr.rgn.indi <-
#       ggplot2::ggplot(data = rr.munc %>% .[Region == x], mapping = ggplot2::aes(x = tmp_lag_03_ordered)) +
#       # null effect
#       ggplot2::geom_hline(mapping = ggplot2::aes(yintercept = 1),
#                           colour = 'black', linetype = 'dashed') +
#       # lines
...
#       gsub(pattern = '[^a-z0-9]', replacement = '', x = .)
#     
#     ggplot2::ggsave(filename = file.path(dir.finalres, sprintf('rr_percentile_region_individual_%s.png', fn.rgn)),
#                     plot = plot.rr.rgn.indi,
#                     height = plot.height,
#                     width = plot.width)
#     
#     
...
# af <-
#   helper.objectLoadRun(
#     filepath = file.path(fp.munc.obsv, 'summary_af.rds'),
#     FUN = NULL,
#     message.run = 'Need to run posterior for municipality at temperature observed',
#     message.load = 'Loading posterior for municipality AF at temperature observed'
#   )
# 
# af.munc <- af$af.municipality
# af.rgn <- af$af.region
# af.ctry <- af$af.country
# 
# ## 4.2. muncipality plot ----
# 
# plot.af.munc <-
#   ggplot2::ggplot(data = af.munc,
#                   mapping = ggplot2::aes(x = year, y = median, group = municipality_id)) +
#   # null effect
#   ggplot2::geom_hline(mapping = ggplot2::aes(yintercept = 1),
#                       colour = 'black', linetype = 'dashed') +
#   # lines
#   ggplot2::geom_line(linewidth = 0.5, alpha = 0.5, colour = 'lightskyblue2') +
#   # axis labels
#   ggplot2::scale_x_continuous(name = 'Year',
#                               limits = c(2011, 2023), breaks = seq(from = 2011, to = 2023, by = 2)) +
#   ggplot2::scale_y_continuous(name = 'Attributable number') +
#   # facet by Region
#   ## make sure facet_geo up to date from GitHuB
#   geofacet::facet_geo(~ Region, grid = italyRegion.grid) +
#   # theme
#   my.theme(legend.position = 'none')
# 
# ggplot2::ggsave(filename = file.path(dir.finalres, 'an_municipality.png'),
#                 plot = plot.af.munc,
#                 height = plot.height,
#                 width = plot.width)
# 
...
# 
# plot.af.rgn <-
#   ggplot2::ggplot(data = af.rgn,
#                   mapping = ggplot2::aes(x = year, y = median)) +
#   # lines
#   ggplot2::geom_line(colour = 'darkorange3') +
...
#   my.theme(legend.position = 'none')
# 
# ggplot2::ggsave(filename = file.path(dir.finalres, 'an_region.png'),
#                 plot = plot.af.rgn,
#                 height = plot.height,
#                 width = plot.width)
# 
...
# 
# plot.af.ctry <-
#   ggplot2::ggplot(data = af.ctry,
#                   mapping = ggplot2::aes(x = year, median)) +
#   # lines
#   ggplot2::geom_line(colour = 'darkorange3') +
#   ggplot2::geom_ribbon(mapping = ggplot2::aes(ymin = lower, ymax = upper),
#                        alpha = 0.25, fill = 'orange') +
...
#   my.theme(legend.position = 'none')
# 
# ggplot2::ggsave(filename = file.path(dir.finalres, 'an_country.png'),
#                 plot = plot.af.ctry,
#                 height = plot.height,
#                 width = plot.width)
# 
# af.ctry %>%
#   .[order(year)] %>%
#   .[, .(year, Country, median, lower, upper)] %>%
#   data.table::fwrite(x = .,
#                      file = file.path(dir.finalres, 'an_country.csv'))
# 
# ## 4.5. clear ----
# 
# rm(
...
# mmt <-
#   helper.objectLoadRun(
#     filepath = file.path(fp.munc.obsv, 'summary_mmt.rds'),
#     FUN = NULL,
#     message.run = 'Need to run posterior for municipality at temperature observed',
#     message.load = 'Loading posterior for municipality MMT at temperature observed'
#   )
# 
# mmt.munc <-
#   mmt %>%
#   .[, .(Code, median, lower, upper)] %>%
#   merge(x = poly.munc,
#         y = .,
#         by.x = 'PRO_COM',
#         by.y = 'Code',
...
# plot.mmt.munc <-
#   ggplot2::ggplot() +
#   ggplot2::geom_sf(data = mmt.munc,
#                    mapping = ggplot2::aes(fill = median),
#                    colour = NA) +
#   ggplot2::scale_fill_gradientn(name = 'Minimum mortality threshold',
#                                 guide = guide_colourbar(title.position = 'top'),
#                                 colors = rev(heat.colors(100))) +
#   my.map.theme(text = element_text(size = plot.textsize),
#                legend.position = 'bottom',
#                legend.title = element_text(hjust = 0.5),
#                legend.key.width = unit(2, 'cm')); plot.mmt.munc
# 
# ggplot2::ggsave(filename = file.path(dir.finalres, 'mmt_municipality.png'),
#                 plot = plot.mmt.munc,
#                 height = plot.height,
#                 width = plot.width)
# 
...
af.munc.obs <-
  helper.objectLoadRun(
    filepath = file.path(fp.munc.obsv, 'posterior_af.rds'),
    FUN = NULL,
    message.run = 'Need to run posterior for municipality at temperature percentile',
    message.load = 'Loading posterior for municipality AF at temperature percentile'
  )

colnames.theta <-
  af.munc.obs %>%
  colnames() %>%
  grep(pattern = 'theta:', x = ., value = TRUE)

## 6.2. region-study period ----

an.rgn <-
  data.table::copy(af.munc.obs) %>%
  # 1. rename for region
  .[, `:=` (
    Code = NA_integer_,
    COMUNE = 'All',
  ...
  # 3. summarise over all exposures for country/region/code/comune/munc_id
  .[, lapply(X = .SD, FUN = sum),
    by = .(Country, Region, Code, COMUNE, municipality_id),
    .SDcols = colnames.theta] %>%
  # 4. summarise
  {
    dt <- .
...
data.table::fwrite(
  x = an.rgn,
  file = file.path(dir.finalres, 'an_region.csv')
)
...
data.table::fwrite(
  x = an.ovr,
  file = file.path(dir.finalres, 'an_overall.csv')
)

# # 99. stage two model prep ----
# 
# ## 99.1. import ----
# 
# rr.munc.perc <-
#   helper.objectLoadRun(
#     filepath = file.path(fp.munc.perc, 'posterior_rr.rds'),
#     FUN = NULL,
#     message.run = 'Need to run posterior for municipality at temperature percentile',
#     message.load = 'Loading posterior for municipality RR at temperature percentile'
#   ) %>%
#   .[, temperaturePercentile := exposure %>% gsub(pattern = '^exposure:', replacement = '', x = .) %>% as.numeric()]
# 
# ## 99.2. filter ----
# 
# rr.munc.perc.select <-
#   rr.munc.perc %>%
#   .[temperaturePercentile %in% c(90, 95, 99)]
# 
# ## 99.3. save ----
# 
# saveRDS(
#   object = rr.munc.perc.select,
#   file = file.path(fp.munc.perc, 'posterior_rr_pecentile_90_95_99.rds')
# )
# 
# ## 99.4. clear ----
# 
