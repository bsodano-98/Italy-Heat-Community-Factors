...
# One can download the mortality data for 2011-2023 at 
...
#Open dataset deaths 2011-2023
deaths <- read_csv("~/Dati/Dataset-decessi-comunali-giornalieri-4/comuni_giornaliero_31ottobre24.csv")
...
deaths <- gather(deaths, agesex, deaths, M_11:F_23, factor_key=TRUE)
...
#Separate each year to create separate datasets from 2011 to 2023

#Run the following code for every year from 2011 to 2023
...
# Replace "n.d." with NA
deaths_2024$deaths[deaths_2023$deaths == "n.d."] <- NA
sum(is.na(deaths_2023$deaths))

deaths_2023 <- deaths_2023 %>% filter(!is.na(deaths))
missing <- deaths_2023 %>% filter(is.na(deaths))

# Aggregate by day and age group
deaths_2023 %>% select(date, COD_PROVCOM, sex, age, deaths, year) %>% 
  group_by(COD_PROVCOM, sex, age, date) %>% 
  summarise(deaths = sum(as.numeric(deaths), na.rm = TRUE)) -> deaths_2023
...
deaths_2023$deaths <- as.numeric(deaths_2023$deaths)
...
deaths_2023 <- deaths_2023 %>%
  ...
  mutate(COD_PROVCOM = "081021",
         NOME_COMUNE = "Trapani",
         year = 2023) %>%
  bind_rows(deaths_2023 %>% filter(!COD_PROVCOM %in% c("081021", "081025")))
#----------------------------------------------------------------------------------
...
# pop17_daily, pop18_daily, pop19_daily, pop20_daily, pop21_daily, pop22_daily, pop23_daily

#Run the following code for every year from 2011 to 2023

#--------------------------------------------------------------------------------
pop23_daily$date <- as.Date(pop23_daily$date)
deaths_2023$date <- as.Date(deaths_2023$date)

deaths_2023 <- deaths_2023 %>%
  select(date, sex, age, COD_PROVCOM, deaths)

# No duplicates
sum(duplicated(deaths_2023[c("date", "sex", "age", "COD_PROVCOM")]))  

# Left join
pop_deaths_2023 <- pop23_daily %>%
  left_join(deaths_2023, by = c("date" = "date", "sex" = "sex", "age" = "age", "Code" = "COD_PROVCOM"))


# Change NA's in 'deaths' with 0
pop_deaths_2023$deaths[is.na(pop_deaths_2023$deaths)] <- 0

#Store the dataset
saveRDS(pop_deaths_2023, file = "~/pop_deaths_XX/pop_deaths_2024.rds")
#--------------------------------------------------------------------------------
#At the end have pop_deaths_2011   #pop_deaths_2016   #pop_deaths_2021
                ...
                #pop_deaths_2014   #pop_deaths_2019   
